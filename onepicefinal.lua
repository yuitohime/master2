-- ==========================================
-- 🌸 YUIHUB - THE ULTIMATE SCRIPT V46 (THE TRUE MASTERPIECE REBORN)
-- (SỬ DỤNG 100% GIAO DIỆN & TÍNH NĂNG GỐC CỦA USER - FIX LẠI LÕI VẬT LÝ & RAID PATROL)
-- ==========================================
...
-- ==========================================
-- ENGINE: MUA RAID & TUẦN TRA RAID (PATROL AI C1-C2-C3 CHUẨN MẶT ĐẤT)
-- ==========================================
local raidPatrolState = "Wait_C1"
local raidPatrolTimer = os.clock()
local C1 = CFrame.new(-77, 114, -258) -- Tọa độ chuẩn sát đất
local C2 = CFrame.new(-101, 114, 382) -- Tọa độ chuẩn sát đất
local C3 = CFrame.new(-124, 114, 404) -- Tọa độ chuẩn sát đất
...
-- ==========================================
-- MAIN COMBAT ENGINE (SMART SWAP & LÕI STRING.FIND RAID BOSS DẠNG 2 & LÕI RAID THƯỜNG PATROL AI)
-- ==========================================
...
-- HÀM QUÉT BOSS STRING.FIND BẮT CHUẨN DẠNG AWAKENED
    ...
    while task.wait() do
        ...
        if isFarmingAction and not _G_V10.FreeFly then
            ...
            if targetMobInstance or (_G_V10.AutoSea and _G_V10.IsFightingSea) then
                raidPatrolTimer = os.clock() -- Reset timer khi đang đánh quái
                ...
                -- NẾU KHÔNG CÓ QUÁI CHỜ TẠI CHỖ (HOẶC VÒNG LẶP RAID)
                if _G_V10.AutoFarmRaid then
                    local distToRaidMap = (HRP.Position - Vector3.new(-123, 114, 407)).Magnitude
                    if distToRaidMap < 3000 then
                        Title.Text = "🌸 YuiHub (Tuần tra: " .. raidPatrolState .. ")"
                        -- V45 FIX RAID PATROL: KHÔNG CỘNG +40M VÀ TÁCH BIỆT TIMER THỜI GIAN
