...
local _G_V10 = {
    ...
    -- PRO Swap Weapon & Specific Skills Feature
...
TabsFrame.BackgroundTransparency = 1; TabsFrame.ScrollBarThickness = 2; TabsFrame.CanvasSize = UDim2.new(0, 0, 0, 550)
...
local function CreateSkillGrid(parent, labelText, varPrefix)
    ...
    Lbl.TextColor3 = Color3.fromRGB(255, 255, 100); Lbl.Font = Enum.Font.GothamBold; Lbl.TextSize = 12; Lbl.TextXAlignment = Enum.TextXAlignment.Left
    
    local Grid = Instance.new("Frame", Container)
    Grid.Size = UDim2.new(1, -10, 0, 25); Grid.Position = UDim2.new(0, 10, 0, 25)
    ...
    for _, key in ipairs(skills) do
        ...
        Btn.Size = UDim2.new(0, 35, 0, 22)
        Btn.BackgroundColor3 = _G_V10[varPrefix..key] and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(80, 80, 80)
        Btn.Text = key; Btn.TextColor3 = Color3.fromRGB(255, 255, 255); Btn.Font = Enum.Font.GothamBold; Btn.TextSize = 12
        Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 4)
        
        Btn.MouseButton1Click:Connect(function()
            _G_V10[varPrefix..key] = not _G_V10[varPrefix..key]
            Btn.BackgroundColor3 = _G_V10[varPrefix..key] and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(80, 80, 80)
        end)
    end
end
...
CreateButton(TabIsland, "🏝️ Quét Danh Sách Đảo", function()
    ...
    else warn("Không tìm thấy đường dẫn Workspace.All.Island!") end
...
CreateButton(TabIsland, "🚀 Bay Đến Đảo / Điểm Hồi Sinh", function()
    ...
    if _G_V10.SelectedIsland then
        ...
        if islandsFolder then
            ...
            if targetIsland then TweenToSafe(targetIsland:GetPivot() + Vector3.new(0, 50, 0)) end
        end
...
-- --- TAB: SETTINGS & PRO WEAPON SWAP SKILLS ---
...
LblDivider.Font = Enum.Font.GothamBold; LblDivider.TextSize = 13; LblDivider.TextXAlignment = Enum.TextXAlignment.Center; LblDivider.Text = "--- AUTO ĐỔI VŨ KHÍ & XẢ SKILL SIÊU TỐC ---"
...
local DropWeapons = CreateDropdown(TabSettings, "Hoặc Chọn 1 Vũ Khí Cố Định", {}, "SelectedWeapon")
...
CreateButton(TabConfig, "💾 Lưu Bản Hiện Tại", function()
    SaveConfig(ConfigNameInput); DropConfigs(GetConfigsList())
end)
...
-- ==========================================
-- ENGINE LÕI (VIM ASYNC FIX)
-- ==========================================
-- FIX: Gọi phím đa luồng không làm nghẽn vòng lặp để xả skill siêu tốc (0.1s)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") then
        ...
        if _G_V10.FreeFly then
            ...
            if not hrp:FindFirstChild("V10_FreeFlyBV") then
                ...
                bv.Name = "V10_FreeFlyBV"; bv.MaxForce = Vector3.new(9e9, 9e9, 9e9)
            end
            if not hrp:FindFirstChild("V10_FreeFlyBG") then
                local bg = Instance.new("BodyGyro", hrp)
                bg.Name = "V10_FreeFlyBG"; bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bg.P = 15000
            end
...
-- ==========================================
-- VÒNG LẶP AUTO FARM CHÍNH & PRO WEAPON SWAP
-- ==========================================
        ...
        if isFarmingAction and not _G_V10.FreeFly then
            ...
            if os.clock() - lastSkillSpamTime >= _G_V10.SkillSpamDelay then
                ...
                if _G_V10.AutoSwapWeapon then
                    ...
                    if _G_V10[prefix.."X"] then PressKey("X") end
                    if _G_V10[prefix.."C"] then PressKey("C") end
                    if _G_V10[prefix.."V"] then PressKey("V") end
...
task.spawn(function()
    while task.wait(0.5) do
        -- Tự Động Play/Join Game Bằng Remote Lấy Từ Hình
    ...
    while task.wait() do 
        ...
        if targetMonster then
            ...
            if targetMonster.Name == "Sea Monster" then
                local radius = 25; local speed = 2; local angle = tick() * speed
            ...
            if _G_V10.IsFightingSea then
                _G_V10.IsFightingSea = false; VIM:SendKeyEvent(false, Enum.KeyCode.W, false, game)
            end
            ...
            if not myBoat then
                ...
                if spawner and spawner:FindFirstChild("LowerTorso") then
                    ...
                    task.wait(0.5)
                    pcall(function() game:GetService("ReplicatedStorage").Assets.Remote.RemoteFunction.Talking:InvokeServer(workspace.NPC.BoatSpawner, workspace.NPC.BoatSpawner, workspace.NPC.BoatSpawner) end)
                    task.wait(1.5)
                end
            else
